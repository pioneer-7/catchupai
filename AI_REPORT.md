...
#### 채점 + 위험도 재계산 (규칙 기반)
- 미니 진단 채점은 AI가 아닌 **정답 키 비교(규칙 기반)** 로 처리
- 정답 수에 따라 위험도 점수 보정 (3문항 정답 → -20점, 2문항 → -10점)
- AI 생성(문항) + 규칙 기���(채점) 분리로 비용 효율과 일관성 확보
...
CatchUp AI는 Claude AI를 **5가지 교육 특화 기능**에 활용하면서, **규칙 기반 분석과 AI 생성을 분리**하는 하이브리드 아키텍처를 채택했습니다. Fallback 체계로 안정성을 확보하고, 스트리밍 UX로 사용자 경험을 극대화했습니다.
